...
- **0.1.c — Meta description longa (3 páginas) — Impacto Baixo · Esforço P — ✅ corrigido no código (2026-06-29; aguarda build/deploy + recrawl).** Encurtadas para ≤160: `/blog` (206→132) em `blog.component.ts`, `/blog/categoria/:slug` (171→140 p/ "Família") no template de `categoria.component.ts`, e `/autor/:slug` (183→136) em `autor.component.ts` (description dedicada e concisa; a `bio` completa segue exibida na página, mas não é mais usada como meta).
- **0.1.d — Title longo (2 páginas) — Impacto Baixo · Esforço P — ✅ corrigido no código + banco (2026-06-29; aguarda build/deploy + recrawl).** Decisão do operador: **sem sufixo de marca em páginas de artigo**. `SeoService` não anexa mais `| Dra. Maria Fernanda Vetere` quando `type==='article'` (o `meta_title` é o `<title>` autoritativo). `/blog` (75→42): título-base passou a conter a marca (`Blog Jurídico | Dra. Maria Fernanda Vetere`), suprimindo o sufixo. Artigo "Traição" (95→49): `meta_title` encurtado no Supabase para "Traição dá direito a indenização? O que diz a lei" (H1/`title` e slug **preservados**). Regra refletida no `MODELO-ARTIGO-BLOG.md` (`meta_title` ≤60, sem sufixo).
- **0.1.e — Redirect chain (1) — Impacto Baixo · Esforço P — ⏸️ sem ação (comportamento inerente da Vercel).** A única cadeia é `http://`apex → `https://`apex → `https://www` (2 saltos). A Vercel força `http→https` no mesmo host **antes** de qualquer redirect de `vercel.json`, então não há como achatar `http://`apex direto em `https://www` num único salto. Os outros 3 "3XX redirect" são saltos únicos saudáveis e esperados (apex→www, `robots.txt`, `http://www`→`https://www`). Impacto de SEO desprezível — aceito como está.
