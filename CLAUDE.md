...
- **OG social dinâmico + separação de imagens (2026-07-01):** decisão de SEO — a imagem **de compartilhamento** (com template/headline) é **separada** da imagem **da página/indexada**. Motivo: o Google Discover pede imagem grande **sem texto sobreposto e sem logo**; texto embutido serve às redes (CTR), não ao Discover. Implementação: (1) `api/og.tsx` — Serverless Function **edge** (`@vercel/og`/Satori) que monta o card 1200×630 (painel `#572c1c` 669.9px à esquerda com logo `mfv-logo-sem-cla-hor.png` a 70%, título Antic Didone centralizado, site Inter a 50%; foto limpa 530.1px à direita) a partir do `slug` (busca `title,coverImage` na `published_articles`). **Zero storage extra** — gerado on-demand, cache no edge. Headline: usa a coluna **`og_headline`** (aditiva em `articles`, exposta como `ogHeadline` na view — migração `add_og_headline_to_articles`) e, se vazia, deriva do `title` (corte na 1ª pontuação). (2) `SeoConfig.ogImage` **desacopla** `og:image`/`twitter:image` do `image` — o `BlogPosting.image` (JSON-LD) continua com a **foto limpa**. (3) `ArtigoComponent` seta `ogImage: ${baseUrl}/api/og?slug=…`. (4) `robots` agora inclui `max-image-preview:large, max-snippet:-1, max-video-preview:-1`. **Foto da página:** `cover_image` migrado de `.png`→`.webp` no Supabase (WebP, CloudConvert Fit=Crop, Strip=Yes, Q80, 1200×630, sem selo/texto). Hero do artigo migrado de `h-[40vh]` para `aspect-[1200/630]` (evita corte no mobile). Dep nova: `@vercel/og`. **Limitação do Satori (motor do `api/og`): não decodifica WebP** (só PNG/JPEG/GIF/SVG). Como a renderização é em streaming, uma imagem não suportada gera um **PNG de 0 bytes** (200, sem erro 500) — não um erro visível. Por isso `api/og` usa o **`.png` irmão** da capa (`coverImage.replace('.webp','.png')`) e embute logo/foto como data-URI com checagem de `content-type` + fallback (302 → `card-blog.png`). **Consequência:** **NÃO apagar os `.png` originais** do Storage — são a fonte do card. **Recomendação durável:** exportar a capa limpa em **JPEG** q80 (Satori lê JPEG; um único arquivo serve página + card + Discover), evitando manter dois formatos. Mapeado no `MODELO-ARTIGO-BLOG.md` (campos `cover_image` limpo + `og_headline`). Detalhar em `BLOG-SEO.md`/`ARCHITECTURE.md`.
